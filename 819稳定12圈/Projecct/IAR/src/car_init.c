...
/*****************************car_init**********************************
* 中断分配�?16个抢占优先级�?0个子优先级，数字越小优先级越�?
*
* PIT定时中断：抢占优先级 1, 响应优先�? 0
* 按键中断：抢占优先级 0�? 响应优先�? 0
* 摄像头回调中断：抢占优先�? 3�? 响应优先�? 0
************************************************************************/
...
void init_gpio()
{
    //初始化GPIO C30 为输�? 默认输出高电�? 使用默认引脚配置GPIO_PIN_CONFIG
...
void init_distance()
{
    //初始换串�?   波特率为115200 TX为B18 RX为B19
    //uart_init (USART_2, 115200,UART2_TX_B18,UART2_RX_B19);
    uart_init(USART_2, 9600, UART2_TX_B18, UART2_RX_B19);
    NVIC_SetPriority(LPUART2_IRQn, 7); //设置串口中断优先�? 范围0-15 越小优先级越�?
...
void init_uart()
{
    uart_init(USART_4, 115200, UART4_TX_D0, UART4_RX_D1); // 
    NVIC_SetPriority(LPUART4_IRQn, 5); //设置串口中断优先�? 范围0-15 越小优先级越�?
...
void init_pit()
{
    ...
    CLOCK_SetDiv(kCLOCK_PerclkDiv, 0U); //分频系数设置�?1
...
void init_debug_uart()
{
    ...
    CLOCK_SetDiv(kCLOCK_UartDiv, 0); //设置UART时钟1分频，即UART时钟�?80Mhz
    ...
    lpuart_config.baudRate_Bps = 460800;
...
void init_buzz()
{
    ...
    //  IOMUXC_SetPinConfig(IOMUXC_GPIO_B0_10_GPIO2_IO10, 0x10B0);
    //
    //  gpioPinConfigStruct.direction = kGPIO_DigitalOutput;
    //  gpioPinConfigStruct.outputLogic = 0;
    //  gpioPinConfigStruct.interruptMode = kGPIO_NoIntmode;
    //
    //  GPIO_PinInit(GPIO2, 10, &gpioPinConfigStruct);
}
...
// void init_light() //frequency : 50Hz
// {
//     pwm_init(PWM2_MODULE1_CHA_C8, 400, 0);
// }